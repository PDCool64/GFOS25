...
public class MessageWS {
  ...
  public Response getMessagesForAccountId(@PathParam("id") int id) {
    ...
    String json = jsonb.toJson(messageFacade.getUnreadMessagesForAccountId(id));
  ...
  public Response getMessagesForChat(@PathParam("senderID") int senderID,
      @PathParam("receiverID") int receiverID, @HeaderParam("Authorization") String token) {
  ...
  public Response getMessageById(@PathParam("id") int id) {
    return responseService.ok(jsonb.toJson(messageFacade.getMessageById(id)));
  }

  @POST
  @Consumes(MediaType.APPLICATION_JSON)
  @Produces(MediaType.APPLICATION_JSON)
  public Response createMessage(@HeaderParam("Authorization") String token, String json) {
    TokenEmail tokenEmail = tokenService.verifyToken(token);
    if (tokenEmail == null || tokenEmail.email == null) {
      return responseService.unauthorized();
    }
    ...
    return responseService.created(jsonb.toJson(messageFacade.createMessage(m)));
  }
}
